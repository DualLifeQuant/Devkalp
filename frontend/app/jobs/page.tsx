...
export default function JobsPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          // Robust Indeed-style fallback to guarantee premium feed experience
          const finalJobs = fetched.length > 0 ? fetched : MOCK_JOBS
          setJobs(finalJobs)
          setTotal(res.data.total || finalJobs.length)
          if (finalJobs.length > 0 && !selectedJob) {
  ...
  useEffect(() => {
    if (filtered.length > 0 && (!selectedJob || !filtered.find(j => j.id === selectedJob.id))) {
          ...
          <div className="flex items-center gap-1.5 flex-wrap w-full md:w-auto py-0.5">
            ...
            {[
              ...
              <button key={f.id} onClick={() => setQuickFilter(f.id)}
                className={clsx(
                  'px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all duration-150 flex items-center gap-1 border shrink-0',
                  quickFilter === f.id
                    ? 'bg-trust-900 text-white border-trust-900 shadow-sm scale-105 font-extrabold'
