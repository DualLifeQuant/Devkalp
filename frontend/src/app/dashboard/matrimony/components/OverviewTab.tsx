...
export default function OverviewTab({
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {profileNotFound && (
        <div className="bg-gradient-to-r from-saffron-50 to-amber-50 border border-saffron-200 rounded-3xl p-5 md:p-6 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex gap-3">
            <div className="w-10 h-10 rounded-2xl bg-saffron-100 flex items-center justify-center shrink-0">
              <AlertCircle className="text-saffron-700" size={20} />
            </div>
            <div>
              <h4 className="font-display text-sm font-bold text-trust-950">Complete Your Matrimony Profile</h4>
              <p className="text-xs text-slate-600 mt-0.5 leading-relaxed max-w-xl">
                You haven't built your matrimony profile yet. Build your profile in the Profile Builder tab to express interest in candidate profiles, receive match recommendations, and connect with senior counselors.
              </p>
            </div>
          </div>
          <Button 
            onClick={() => onSwitchTab('profile')} 
            className="w-full md:w-auto bg-saffron-500 text-white hover:bg-saffron-600 font-bold px-6 py-2.5 rounded-2xl text-xs flex items-center justify-center gap-1.5 shrink-0 shadow-warm border-none"
          >
            Build Profile Now <ArrowRight size={14} />
          </Button>
        </div>
      )}
