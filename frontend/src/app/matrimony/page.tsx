...
const FAQ_ITEMS = [
  ...
  { q: 'Is the matrimony service free to use?', a: 'Registration, profile creation and The full counselor-guided service are 100% free.' },
  { q: 'How does the matching process actually work?', a: 'Our counselors review your profile, compatibility expectations, and family details to manually identify suitable matches. While we guide the process carefully, background verification remains the responsibility of the individuals involved. No algorithm — every suggestion is made by a human.' },
...
export default function MatrimonyPage() {
  ...
  const [hasMatrimonyProfile, setHasMatrimonyProfile] = useState<boolean | null>(null)
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      ...
      matrimonyApi.getMyProfile()
        .then(() => setHasMatrimonyProfile(true))
        .catch((err) => {
          if (err?.response?.status === 404) {
            setHasMatrimonyProfile(false)
          } else {
            setHasMatrimonyProfile(false)
          }
        })
              ...
              {isMounted && isLoggedIn ? (
                <>
                  ...
                  {hasMatrimonyProfile === true ? (
                    ...
                    <motion.div className="inline-flex items-center gap-2 px-7 py-3.5 bg-white/10 text-white/50 font-medium rounded-xl border border-white/10 text-sm cursor-wait select-none">
                      Checking Profile...
                    </motion.div>
                  )}
                </>
          ...
          <section className="relative z-30 bg-white border-b border-slate-200/80 shadow-sm w-full transition-all">
            <div className="page-container py-4">
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner shrink-0 items-center relative z-0">
                  {(['Bride', 'Groom'] as const).map(v => {
                    const active = filters.lookingFor === v
                    return (
                      <button key={v} onClick={() => setFilter('lookingFor', v)}
                        className={clsx('relative px-5 py-2 text-xs font-bold rounded-lg transition-colors duration-200 flex items-center gap-1.5 z-10',
                          active ? 'text-white' : 'text-slate-600 hover:text-trust-900')}>
                        {active && (
                          <motion.span
                            layoutId="activeLookingForBg"
                            className="absolute inset-0 bg-trust-900 rounded-lg shadow-sm -z-10"
                            transition={{ type: 'spring', stiffness: 350, damping: 25 }}
                          />
                        )}
                        {v === 'Bride' ? <Heart size={13} className={active ? 'fill-saffron-300 text-saffron-300' : ''} /> : <UserCheck size={13} />}
                        {v}
                      </button>
                    )
                  })}
              ...
              <div className="mt-3 pt-3 border-t border-slate-100 flex items-center gap-3 overflow-x-auto flex-nowrap w-full pb-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {/* Category Tabs */}
                  <div className="flex bg-slate-100/80 p-1 rounded-xl shadow-inner shrink-0 items-center relative z-0">
                    {FILTER_TABS.map(tab => {
                      const active = activeTab === tab
                      const hasActiveFilter = isTabActive(tab)
                      return (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                          className={clsx('relative px-3.5 py-1.5 text-xs font-bold rounded-lg transition-colors duration-200 flex items-center gap-1.5 shrink-0 z-10',
                            active ? 'text-trust-950 font-extrabold' : 'text-slate-600 hover:text-slate-900')}>
                          {active && (
                            <motion.span
                              layoutId="activeFilterTabBg"
      ...
      <section className="py-16 bg-white">
        <div className="page-container">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {[
              { couple: 'Priya & Rohan', year: '2025', city: 'Surat', story: 'The counselor took time to understand both families. Within three months we knew this was right.', img: 'success_1.jpg' },
              { couple: 'Kavya & Ankit', year: '2025', city: 'Surat', story: 'The counselor\'s detailed compatibility matching helped us align on all major life expectations early.', img: 'success_2.jpg' },
              { couple: 'Nisha & Dev', year: '2025', city: 'Ahmedabad', story: 'Our families were involved from the beginning, which made the entire process feel natural and dignified.', img: 'success_3.jpg' },
            ].map((s, i) => (
              ...
              {isMounted && isLoggedIn ? (
                <>
                  ...
                  {hasMatrimonyProfile === true ? (
                    ...
                    <div className="inline-flex items-center gap-2 px-8 py-3.5 bg-white/10 text-white/50 font-medium rounded-xl border border-white/10 text-sm cursor-wait select-none">
                      Checking Profile...
                    </div>
                  )}
                </>
