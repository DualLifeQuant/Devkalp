...
import { UserCheck, Calendar, Users, Plus, X, Check } from 'lucide-react'
...
import { useCounselorSessions, useAddSessionNotes } from '@/hooks/useApiQueries'
...
export default function CounselorSessionsPage() {
  ...
  const { data: sessions = [], isLoading: loading, isError: sessionsError, refetch: refetchSessions } = useCounselorSessions(params)
  const addNotesMutation = useAddSessionNotes()

  const saveNotes = async () => {
    if (!selected) return
    try {
      await addNotesMutation.mutateAsync({ id: selected.id, data: { notes, recommendations: recs, status: 'completed' } })
      toast.success('Session notes saved')
      setSelected(null)
    } catch { toast.error('Failed to save notes') }
  }

  const upcoming = (sessions as any[]).filter(s => s.status === 'scheduled')
  ...
  return (
    <DashboardLayout navItems={NAV} title="Counselor">
      <div className="p-6 md:p-8 space-y-6">
        ...
        {sessionsError && <InlineError message="Failed to load sessions." onRetry={() => refetchSessions()} className="mb-4" />}
        {loading ? (
          ...
          <div className="bg-white rounded-3xl shadow-float w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {selected.user_name} · {new Date(selected.session_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                </p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3">
                <Button onClick={saveNotes} loading={addNotesMutation.isPending} variant="sage" className="flex-1 justify-center">
