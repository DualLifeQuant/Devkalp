...
import { useMyApplications } from '@/hooks/useApiQueries'

const NAV = [
  { href: '/dashboard/jobs', icon: <Briefcase size={18} />, label: 'My Applications' },
  { href: '/jobs',           icon: <ChevronRight size={18} />, label: 'Browse Jobs' },
]
...
export default function CandidateDashboard() {
  useAuthGuard({ allowedRoles: ['candidate', 'admin'] })

  const { data: apps = [], isLoading: loading, isError, refetch } = useMyApplications()
  const active = (apps as any[]).filter((a: any) => !['rejected','withdrawn'].includes(a.status))
  const interviews = (apps as any[]).filter((a: any) => a.status === 'interview_scheduled')

  return (
    <DashboardLayout navItems={NAV} title="Careers">
      <div className="p-6 md:p-8 space-y-8">
        ...
        {isError && <InlineError message="Failed to load applications." onRetry={() => refetch()} className="mb-4" />}
