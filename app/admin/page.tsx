...
import { StatsCard, Badge, Card, EmptyState } from '@/components/ui'
import { SkeletonStats, SkeletonList } from '@/components/common/LoadingStates'
import { useAuthGuard } from '@/hooks/useAuthGuard'
import { useAdminStats, useAdminMatrimonyProfiles, useAdminApplications } from '@/hooks/useApiQueries'
...
export default function AdminDashboard() {
  ...
  const { data: pendingData, isLoading: profilesLoading } = useAdminMatrimonyProfiles({ status: 'pending', limit: 5 })
  const { data: recentAppsData, isLoading: appsLoading } = useAdminApplications({ limit: 5 })

  const pendingProfiles = (pendingData as any)?.items ?? []
  const appsList = recentAppsData ?? []
  const loading = statsLoading || profilesLoading || appsLoading
      ...
      <div className="p-6 md:p-8 space-y-7">
        <FadeIn>
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/activity" className="hidden md:flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 text-slate-600 text-sm hover:border-trust-300 hover:text-trust-700 transition-all">
        ...
        {loading ? (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <SkeletonList count={3} /><SkeletonList count={3} />
            </div>
          </div>
        ) : (
          <>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {stats.pending_matrimony_profiles > 0 && (
                      <Link href="/admin/matrimony" className="inline-flex items-center gap-1.5 bg-amber-100 text-amber-800 text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-amber-200 transition-colors">
                        {stats.pending_matrimony_profiles} matrimony profiles to review <ArrowUpRight size={12} />
                      </Link>
                    )}
                    {stats.pending_applications > 0 && (
                      <Link href="/admin/jobs" className="inline-flex items-center gap-1.5 bg-amber-100 text-amber-800 text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-amber-200 transition-colors">
                        {stats.pending_applications} job applications pending <ArrowUpRight size={12} />
                      </Link>
                    )}
                    {stats.pending_volunteers > 0 && (
                      <Link href="/admin/volunteers" className="inline-flex items-center gap-1.5 bg-amber-100 text-amber-800 text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-amber-200 transition-colors">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <FadeIn key={s.label} delay={0.1 + i * 0.06}>
                  <motion.div whileHover={{ y: -3 }} transition={{ duration: 0.2 }}><StatsCard {...s} /></motion.div>
                </FadeIn>
              ))}
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <FadeIn delay={0.35}>
                <Card className="p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <Link href="/admin/matrimony" className="text-xs text-trust-600 hover:text-trust-800 font-medium flex items-center gap-1">View all <ArrowUpRight size={12} /></Link>
                  </div>
              ...
              <FadeIn delay={0.42}>
                <Card className="p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <Link href="/admin/jobs" className="text-xs text-trust-600 hover:text-trust-800 font-medium flex items-center gap-1">View all <ArrowUpRight size={12} /></Link>
                  </div>
                  {appsList.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {QUICK_ACTIONS.map((a) => (
                    <motion.div key={a.href} whileHover={{ y: -3, scale: 1.02 }} whileTap={{ scale: 0.97 }} transition={{ duration: 0.2 }}>
                      <Link href={a.href} className={`flex items-center gap-2.5 p-4 rounded-2xl border text-sm font-semibold transition-all ${a.color}`}>
                        {a.icon} {a.label}
                      </Link>
                    </motion.div>
                  ))}
