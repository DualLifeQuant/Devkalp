...
import { Badge, Card, EmptyState, StatsCard, Button } from '@/components/ui'
import { SkeletonStats, SkeletonList, InlineError} from '@/components/common/LoadingStates'
import { useAuthGuard } from '@/hooks/useAuthGuard'
import { useAdminDonations, useDonationCampaigns, useCreateDonationCampaign } from '@/hooks/useApiQueries'
...
export default function AdminDonationsPage() {
  ...
  const { data: donationsData, isLoading: donationsLoading } = useAdminDonations({ limit: 100 })
  const { data: campaignsData, isLoading: campaignsLoading } = useDonationCampaigns({ limit: 50 })
  const loading = donationsLoading || campaignsLoading

  const donations: any[] = (donationsData as any)?.items || []
  const stats = { total: (donationsData as any)?.total_amount || 0, count: (donationsData as any)?.total || 0 }
  ...
  const createCampaign = async () => {
    ...
    try {
      await createCampaignMutation.mutateAsync({ ...newCampaign, target_amount: newCampaign.target_amount ? parseFloat(newCampaign.target_amount) : null })
  ...
  return (
    <AdminLayout>
      <div className="p-6 md:p-8 space-y-6">
        ...
        {loading ? <div className="space-y-4"><SkeletonStats count={2}/><SkeletonList count={4}/></div> : (
          <>
            {tab==='donations' && (
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={()=>setShowNewCampaign(false)}>
          <div className="bg-white rounded-3xl shadow-float w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <Button onClick={createCampaign} loading={createCampaignMutation.isPending} className="w-full justify-center">Create Campaign</Button>
            </div>
          </div>
        </div>
      )}
